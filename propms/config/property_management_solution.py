...
@frappe.whitelist()
def my_method(docname, *args, **kwargs):
    print(docname)
    last_meter_reading = frappe.db.get_value("Meter Reading", filters={"docstatus": 1}, fieldname=["name"], order_by="creation DESC")
    if last_meter_reading:
        last_meter_reading_doc = frappe.get_doc("Meter Reading", last_meter_reading[0])
        for i, current_reading in enumerate(docname.meter_reading_detail):
            last_reading = last_meter_reading_doc.meter_reading_detail[i]
            previous_reading_difference = last_reading.reading_difference
            current_reading.previous_reading_difference = previous_reading_difference